...
img_file_buffer = st.file_uploader('Upload an image', type=['png', 'jpg', 'jpeg'])  # Added a non-empty label
...
if img_file_buffer is not None:
    ...
    with st.spinner("Wait for it..."):
        ...
        files = {'img': ('uploaded_image.jpg', img_bytes, 'image/jpeg')}
  # Simplified format
        res = requests.post(url + "/upload_image", files=files)
        # 500 request status code. here is the problem.
        st.write(res.status_code)
        # Make request to API
        try:
            files = {'img': img_bytes}  # Simplified format
            res = requests.post(url + "/upload_image", files=files)
